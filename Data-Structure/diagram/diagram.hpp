...
namespace ds {
    ...
    private:
        void topoSortDFSHelp(int64_t index_dot, std::vector<int64_t> &stk) {
            ...
            typename AdjListDirNet<Elem>::vexLinkNode *p = this->m_vexs[index_dot].m_firstArc->m_next;
            while (p) {
    ...
    public:
            ...
            for (size_t i = 0; i < res.size(); i++) {
                for (size_t j = 0; j < i; j++) {
                    ...
                    if (this->m_vexs[res[j]][res[i]])
                         ...
                         */
                        // 更新顶点 res[i] 的路径长度为顶点 res[j] 的路径长度加 res[1] 或者不变的较大值
                        path[res[i]] = path[res[i]] > path[res[j]] + 1 ? path[res[i]] : path[res[j]] + 1;
                }
            }
            size_t maxPath{0};
        ...
        int64_t exit_path_number(int64_t index_dot1, int64_t index_dot2, int64_t k) const {
            ...
            return res[index_dot1][index_dot2];
        }
    };
