...
// 有向图的邻接表
template <typename Elem> class AdjListNet {
protected:
  struct vexLinkNode {
    int64_t m_adjVex{-1};
    int64_t m_weight{};
...
template <typename Elem> class AdjListDirNet : public AdjListNet<Elem> {
  // 有向图的邻接表
public:
...
template <typename Elem> class AdjListUndirNet : public AdjListNet<Elem> {
  // 无向图的邻接表
public:
...
template <typename Elem> class AdjMatrixNet {
  // 邻接矩阵
protected:
...
public:
  ...
  AdjMatrixNet(const Elem *es, size_t vertexnum)
      : m_data(es, es + vertexnum), ArcNum(0), visited(vertexnum, false),
...
template <typename Elem> class AdjMatrixUnDirNet : public AdjMatrixNet<Elem> {
  // 无向图的邻接矩阵
public:
...
template <typename Elem> class AdjMatrixDirNet : public AdjMatrixNet<Elem> {
  // 无向图的邻接矩阵
public:
  using AdjMatrixNet<Elem>::AdjMatrixNet;
  ...
  auto topologicalSort() const {
    ...
    std::vector<int64_t> inDegree(this->m_data.size(), 0); // 记录每个顶点的入度
    ...
    for (size_t i = 0; i < this->m_vexs.size(); i++) {
      for (size_t j = 0; j < i; j++) {
        ...
        if (this->m_vexs[res[j]][res[i]])
           ...
           */
          // 更新顶点 res[i] 的路径长度为顶点 res[j] 的路径长度加 res[1]
          // 或者不变的较大值
          // NOTE:带权图时把 1 替换为权值 this->m_vexs[res[j]][res[i]]
          path[res[i]] =
              path[res[i]] > path[res[j]] + this->m_vexs[res[j]][res[i]]
                  ? path[res[i]]
  ...
  int64_t exit_path_length(int64_t index_dot1, int64_t index_dot2,
     ...
     * 使用快速幂得到 k 次幂的矩阵
     */
    if (index_dot1 < 0 || index_dot2 < 0 || index_dot1 >= this->m_vexs.size() ||
    ...
    if (k == 1)
      return this->m_vexs[index_dot1][index_dot2];

    auto martixMultiply = [](const std::vector<std::vector<int64_t>> &mtrix_1,
