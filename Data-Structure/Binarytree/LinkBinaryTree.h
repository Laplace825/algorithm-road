...
// 各种声明
template <class Elem> class LinkBinaryTree;
template <class Elem> class BinTreeNode;
...
template <class Elem> unsigned int leafNum(const LinkBinaryTree<Elem> &root);
// 定义
template <class Elem> class BinTreeNode {
...
template <class Elem> LinkBinaryTree<Elem>::LinkBinaryTree() : root(nullptr) {}
...
template <class Elem> LinkBinaryTree<Elem>::~LinkBinaryTree() {
  delete root;
  root = nullptr;
  /**
...
template <class Elem> void LinkBinaryTree<Elem>::levelOrder() const {
  std::deque<BinTreeNode<Elem> *> que;
  if (root != nullptr)
    que.push_back(root);
  while (!que.empty()) {
...
template <class Elem> void LinkBinaryTree<Elem>::preOrder() const {
  // // 使用栈非递归前序遍历
...
template <class Elem> void LinkBinaryTree<Elem>::inOrder() const {
  // 使用栈非递归中序遍历
  // 使用标志位记录结点是否被访问过
  // std::deque<Uint> flag;
  // 这里是因为当左子树全遍历完后,stk和flag都空,此时flag.back()会出错,所以先push_back一个0
...
template <class Elem> void LinkBinaryTree<Elem>::postOrder() const {
  /**
   * @todo : 完成非递归后序遍历
   * */
  // 使用栈非递归中序遍历
...
template <class Elem>
typename LinkBinaryTree<Elem>::Uint
LinkBinaryTree<Elem>::depth(const BinTreeNode<Elem> *rt) const {
  if (rt == nullptr)
    return 0;
  else {
...
template <class Elem>
typename LinkBinaryTree<Elem>::Uint
LinkBinaryTree<Elem>::leafNum(const BinTreeNode<Elem> *rt) const {
  if (rt == nullptr)
    return 0;
...
typename LinkBinaryTree<Elem>::Uint LinkBinaryTree<Elem>::leafNum() const {
  ...
  if (root != nullptr)
    que.push_back(root);
  while (!que.empty()) {
    if (que.front()->leftChild == nullptr && que.front()->rightChild == nullptr)
      leafNum++;
    if (que.front()->leftChild != nullptr)
      que.push_back(que.front()->leftChild);
    if (que.front()->rightChild != nullptr)
...
template <class Elem>
BinTreeNode<Elem> *&
LinkBinaryTree<Elem>::createHelp(BinTreeNode<Elem> *&rt, const Elem pre[],
                                 const Elem in[], int pleft, int pright,
                                 int ileft, int iright) {
  if (ileft > iright)
    rt = nullptr;
    ...
    createHelp(rt->leftChild, pre, in, pleft + 1, pleft + mid - ileft, ileft,
               mid - 1);
...
template <class Elem> auto LinkBinaryTree<Elem>::getRoot() const noexcept {
  return this->root;
}
