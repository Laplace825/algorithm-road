...
namespace tree
{
    ...
    protected:
        ...
        static void print(Iterator &&begin, Iterator &&end, PrintType &&print)
        {
            try
            {
                ...
                for (auto it = begin; it != end; it++)
                    std::cout << std::format("{:^2} |", static_cast<decltype(print)>(*it));
                std::cout << '\n';
            }
        ...
        static constexpr std::size_t degree(Container &&container)
        {
            auto maxVal = [](const auto &cont) -> int64_t
            {
                ...
                for (auto &it : cont)
                {
                    if (static_cast<int64_t>(it) > maxIndex)
                        maxIndex = it;
                }
                return maxIndex;
            };
        ...
        //     }
        //     /**
        //      * @note: if you count the parent node, you can give up the loop below
        //      *        just put the parentNum++ under the code `counter[parent] = true;`
        ...
        //          it != container.end() && *(unique.end() - 1) != static_cast<int64_t>(*(container.end() - 1));
        //          it++)
        //     {
        //         /**
        ...
        //      * to understand,please think about the parent_index already implies the parents exist
        //      * so when we finish unqiue operation, just counting who wasn't root is enough
        //      */
        ...
        static constexpr std::size_t leafNum(Container &&container)
        {
             ...
             *          space complexity is O(n),time complexity is O(n) (bad case is O(n^2))
             */
            return container.size() - std::unordered_set<int64_t>(container.begin() + 1, container.end()).size();
        }
    };
    ...
    protected:
        struct Node
        {
            ...
            int64_t m_parent;
            Node() = default;
            explicit Node(const ElemT &data, int64_t parent) : m_data(data), m_parent(parent) {}
    ...
    protected:
        ...
        struct CNode
        {
            ...
            explicit CNode(std::size_t index, CNode *nxt) : m_childindex(index), next(nxt) {}
    ...
    public:
        ...
        std::size_t leafNum() noexcept
        {
            ...
            for (auto &it : m_node)
            {
                if (it.m_childlink->next == nullptr)
                    leafNum++;
            }
            return leafNum;
        }
